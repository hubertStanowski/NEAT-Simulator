...
const NavItems: React.FC<NavItemsProps> = ({
  ...
  return (
    <ul className="nav-ul ml-1 gap-[clamp(1rem,3vw,3.5rem)] sm:ml-2">
      <a
        className={`nav-li_a text-[clamp(1rem,2.5vh,1.5rem)] ${selectedSimulation === "Snake" ? "text-purple-700" : "text-neutral-400"}`}
        onClick={() => {
      ...
      <a
        className={`nav-li_a text-[clamp(1rem,2.5vh,1.5rem)] ${selectedSimulation === "FlappyBird" ? "text-purple-700" : "text-neutral-400"}`}
        onClick={() => {
          alert(
